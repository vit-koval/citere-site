...
Nothing on these pages is a ranking. A product that declines every question repeats nothing and helps nobody, so declined answers are counted separately rather than treated as corrections. We test the consumer interface, not the API, and record the model version each product reported at the time.
