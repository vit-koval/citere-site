...
- **Escalations → Countermeasures.** `/countermeasures/`, `data/countermeasures.json`, `countermeasures.csv`; `/escalations/` is a redirect stub (`src/_data/redirects.js`). The twelve types of the catalogue §2.1–2.12, the one lifecycle `drafted → pending_confirmation → submitted → acknowledged → responded → closed → declined`, and the §4 ladder live in `src/_lib/labels.cjs`; `src/_data/countermeasures.js` maps the exporter's older vocabulary onto them. **A re-measurement is not a type**: the catalogue puts it inside the disclosure lifecycle (§2.1 "Track", `follow_up_due` = submission + 14 days, feeding CM §7), so it is carried as `kind: "remeasurement"` on a disclosure row — it gets its own column wherever a page asks for one, and it never counts towards "countermeasures taken". `submission_content`, `proof`, `target_contact` and `confirmed_by` are internal: the schema forbids them in `data/`, and `check.mjs` fails if they reach the build.
