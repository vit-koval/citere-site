...
- **Metrics store.** `data/metrics.json` — one cell per bot × claim × persona × market × run (CM §5.1) — plus `data/runs.json`, one record per run × market, since a run is one country and one language (CM §0.2). Derived by `scripts/build-metrics.mjs` (`npm run metrics`), rebuilt by the importer, re-derived by `npm run validate`, which fails on drift and on a set of arithmetic assertions (SR §12).
- **The rules are in the code, not in the discipline of each page.** `src/_data/metrics.js`: counts may be summed across claims and bots; a *rate* only forms inside one persona, one market and one run, otherwise it returns `null` with the reason in `blocked` (CM §5.1). Two denominators, never interchanged — Repeat Rate over substantive answers, Contamination Rate and the verdict distribution over all valid answers (CM §5.2–5.5). `quarantine` and `UNRESOLVED` are counted and then excluded (CM §0.3). Wilson at z = 1.96, `low_n` below 20, significance only by non-overlap (CM §5.6). The grain-of-truth split runs on `splice`, never on the `grain_of_truth` field (CM §5.7).
- **Layer B.** Watchlist matching is exact or subdomain on a dot boundary, with a mandatory exclusion list — `pravda.com.ua` is Ukrainska Pravda and must never match (CM §3.2, pipeline guide §4.1). Networks fold onto the three categories `pravda_network > state_media > laundering_network`. One response counts once per domain (CM §5.8).
- **Escalations → Countermeasures.** `/countermeasures/`, `data/countermeasures.json`, `countermeasures.csv`; `/escalations/` is a redirect stub (`src/_data/redirects.js`). Types and the `drafted → … → closed` lifecycle live in `src/_lib/labels.cjs`; `src/_data/countermeasures.js` maps the exporter's older vocabulary onto them.
...
- `Citere_Countries_Index_Business_Logic.md` and `Citere_Countermeasures_Catalogue.md` are still missing. The twelve countermeasure types come from the brief and the reference builds; `remeasurement` is carried as a thirteenth because the Claim Report Spec, the Countries pages and CM §7 all treat it as a type of its own. Reconcile when the catalogue lands.
- The export carries no `splice`, `debunk_argument`, `prompt_id`, `repeat_n`, `is_live`, `dodge_type`, `acknowledged_grain`, quarantine or UNRESOLVED status, and records only watchlisted citations rather than all cited domains. The store reads every one of them and tolerates null, so nothing breaks — but stability (CM §5.11), retrieval share, the grain-of-truth split and the live-vs-grid check cannot be computed until they arrive.
- `data/benchmarks.json` still splits grain of truth by the `grain_of_truth` field, which CM §5.7 rules out. It cannot be recomputed until claims carry `splice`; the Benchmarks page is rebuilt at the brief's step 7.
